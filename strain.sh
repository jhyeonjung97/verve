...
if [[ -z $1 ]]; then
    pwd
    read -p 'Enter the name of the structure file: ' file
else
    file=$1
fi
...
python ~/bin/verve/cell-size.py -f -0.3 -o "${name}1$ext"
python ~/bin/verve/cell-size.py -f -0.2 -o "${name}2$ext"
python ~/bin/verve/cell-size.py -f -0.1 -o "${name}3$ext"
python ~/bin/verve/cell-size.py -f 0.0 -o "${name}4$ext"
python ~/bin/verve/cell-size.py -f 0.1 -o "${name}5$ext"
python ~/bin/verve/cell-size.py -f 0.2 -o "${name}6$ext"
python ~/bin/verve/cell-size.py -f 0.3 -o "${name}7$ext"
